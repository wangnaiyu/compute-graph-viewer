import {
  ...
  apiLoad,
  apiRangeProgress,
  defaultApiParameterDefinitions,} from './memstates';
...
export const createShiftRightControlStates = ({
  blockCount = 4,
  blockLen = 2,
  srcStride = 0,
  dstStride = 0,
    ...
    process: {
      ...
      transfer: () => {
        ...
        for (let i = 0; i < blockLen; i += 1) {
          dstData[dstOffset + i] = src0Data[srcOffset + i] >> scalarValue;
        }
...
export const shiftRightApiParameterDefinitions = [
  ...defaultApiParameterDefinitions,
  { id: 'scalarValue', label: 'shift-bit(位移)', min: 0, max: 15, defaultValue: 1 },
];

export const shiftRightApiDefinition = {
  id: 'shiftright',
  label: 'ShiftRight',
  createControlStates: createShiftRightControlStates,
  parameterDefinitions: shiftRightApiParameterDefinitions,
  dataColors,
  apiNodeDefinitions,
};
